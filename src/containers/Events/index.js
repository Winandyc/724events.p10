...
const EventList = () => {
  ...
  const filteredEvents = (
    ...
    ) ?? [] // si TYPE est défini, les événements sont filtrés pour ne retourner que ceux dont le type correspond à la valeur de TYPE.
    // l'opérateur ?? [] est utilisé pour s'assurer qu'en cas de valeur "null" ou "undefined", un tableau vide est retourné plutôt qu'une valeur null ou undefined. Cela garantit que filteredEvents soit toujours un tableau, même s'il n'y a pas d'événements à retourner.
  ).filter((_, index) => { // je filtre les événements selon leur index dans le tableau filtré "data?.events" (_ = "event" est déclaré mais n'est jamais lue).
    ...
    return startIndex <= index && index < endIndex; // vérifie si l'indice de l'événement est dans la plage à afficher sur la page actuelle en utilisant les indices de début et de fin calculés précédemment.
  });
